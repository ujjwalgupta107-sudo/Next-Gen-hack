 ...
 */
export function parseTask(cliArguments, usedCliArguments, tasksMap) {
    const taskOrId = getTaskFromCliArguments(cliArguments, usedCliArguments, tasksMap);
    return taskOrId;
}
function getTaskFromCliArguments(cliArguments, usedCliArguments, tasksMap) {
    const taskId = [];
    let task;
    for (let i = 0; i < cliArguments.length; i++) {
...
function validateRequiredArguments(argumentDefinitions, taskArguments) {
    const missingRequiredArgument = argumentDefinitions.find(({ defaultValue, name, type }) => isArgumentRequired(type, defaultValue) &&
