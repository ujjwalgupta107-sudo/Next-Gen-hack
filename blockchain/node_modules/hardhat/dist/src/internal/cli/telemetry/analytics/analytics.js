...
import { isTelemetryAllowedInEnvironment, isTelemetryAllowed, } from "../telemetry-permissions.js";
...
export async function sendTelemetryConfigAnalytics(enabled) {
...
export async function sendTaskAnalytics(taskId, command) {
...
export async function sendProjectTypeAnalytics(template) {
    const initAnalyticsEvent = {
        name: "init",
        params: {
...
export async function sendNetworkAnalytics(chainType) {
    const networkAnalyticsEvent = {
        name: "network",
        params: {
            chainType,
        },
...
async function sendAnalytics(analyticsEvent) {
...
async function createSubprocessToSendAnalytics(payload) {
    log(`Sending analytics for '${payload.events[0].name}'. Payload: ${JSON.stringify(payload)}`);
    ...
    const fileExt = process.env.HARDHAT_TEST_SUBPROCESS_RESULT_PATH !== undefined ? "ts" : "js";
...
async function buildPayload(analyticsEvent) {
