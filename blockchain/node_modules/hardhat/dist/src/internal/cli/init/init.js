...
import { copy, ensureDir, exists, getAllFilesMatching, isDirectory, mkdir, readJsonFile, remove, symlink, writeJsonFile, writeUtf8File, } from "@nomicfoundation/hardhat-utils/fs";
...
import { getHardhatVersion, getLatestHardhatVersion, } from "../../utils/package.js";
...
import { getDevDependenciesInstallationCommand, getPackageManager, getVersion, installsPeerDependenciesByDefault, } from "./package-manager.js";
import { promptForMigrateToEsm, promptForForce, promptForInstall, promptForTemplate, promptForUpdate, promptForWorkspace, } from "./prompt.js";
import { spawn } from "./subprocess.js";
import { getTemplates } from "./template.js";
const log = createDebug("hardhat:core:cli:init");
export async function initHardhat3NonInteractive(options) {
    const [template, projectTypeAnalyticsPromise] = await getTemplate(options.template, true);
    const workspace = process.cwd();
    try {
        const configFilePath = await findClosestHardhatConfig(workspace);
        throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.HARDHAT_PROJECT_ALREADY_CREATED, {
            hardhatProjectRootPath: configFilePath,
        });
    }
    catch (err) {
        if (!HardhatError.isHardhatError(err, HardhatError.ERRORS.CORE.GENERAL.NO_CONFIG_FILE_FOUND)) {
            throw err;
        }
    }
 ...
 */
export async function initHardhat(options) {
    try {
        printAsciiLogo();
        ...
        const [template, projectTypeAnalyticsPromise] = await getTemplate(options?.template);
        // Create the package.json file if it does not exist
        // and validate that it is an esm package
        await validatePackageJson(workspace, template.packageJson, options?.migrateToEsm);
...
export async function printWelcomeMessage() {
    ...
    console.log(styleText("cyan", `👷 Welcome to ${HARDHAT_NAME} v${hardhatVersion} 👷\n`));
    ...
    try {
        ...
        if (hardhatVersion !== latestHardhatVersion) {
            console.warn(styleText(["yellow", "bold"], `⚠️ You are using an outdated version of Hardhat. The latest version is v${latestHardhatVersion}. Please consider upgrading to the latest version before continuing with the project initialization. ⚠️\n`));
        }
    }
    catch (error) {
        ensureError(error);
        ...
        console.warn(styleText(["yellow", "bold"], `⚠️ We couldn't check if you are using the latest version of Hardhat. Please consider upgrading to the latest version if you are not using it yet. ⚠️\n`));
    }
}
/**
...
export async function getWorkspace(workspace) {
    ...
    if ((await exists(workspace)) && !(await isDirectory(workspace))) {
        throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.WORKSPACE_MUST_BE_A_DIRECTORY, {
            workspace,
        });
    }
 ...
 */
export async function getTemplate(template, includeAvailableTemplatesInErrors = false) {
    ...
    throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.TEMPLATE_NOT_FOUND_WITH_LIST_OF_OPTIONS, {
        template,
        availableTemplates,
    });
}
 ...
 */
export async function printTemplatesList(print = console.log) {
    const templates = await getTemplates();
 ...
 * @param workspace The path to the workspace to initialize the project in.
 */
export async function validatePackageJson(workspace, templatePkg, migrateToEsm) {
    ...
    const pkg = await readJsonFile(absolutePathToPackageJson);
    ...
    await spawn([packageManagerToUse, "pkg", "set", "type=module"].join(" "), [], {
        cwd: workspace,
        shell: true,
        stdio: "inherit",
    });
 ...
 */
export function relativeWorkspaceToTemplatePath(file) {
    if (path.basename(file) === ".gitignore") {
        return path.join(path.dirname(file), "gitignore");
    }
    return file;
}
export function relativeTemplateToWorkspacePath(file) {
    if (path.basename(file) === "gitignore") {
        return path.join(path.dirname(file), ".gitignore");
    }
    return file;
 ...
 */
export async function copyProjectFiles(workspace, template, force) {
    // Find all the paths in the template that clash with an existing one in the
    // workspace
    const matchingRelativeWorkspacePaths = (await Promise.all(template.files.map(async (relativeTemplatePath) => {
        const relativeWorkspacePath = relativeTemplateToWorkspacePath(relativeTemplatePath);
    ...
    const matchingRelativeWorkspacePathsSet = new Set(matchingRelativeWorkspacePaths);
...
// NOTE: This function is exported for testing purposes
export async function assertNoNonInteractiveClashes(workspace, template) {
    const clashes = [];
    for (const relativeTemplatePath of template.files) {
        const relativeWorkspacePath = relativeTemplateToWorkspacePath(relativeTemplatePath);
        if (relativeWorkspacePath === "package.json" ||
    ...
    throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.NON_INTERACTIVE_INIT_WOULD_OVERWRITE_FILES, {
        files: clashes.map((f) => `  - ${f}`).join("\n"),
    });
}
// NOTE: This function is exported for testing purposes
export async function copyProjectFilesNonInteractive(workspace, template) {
    for (const relativeTemplatePath of template.files) {
        const relativeWorkspacePath = relativeTemplateToWorkspacePath(relativeTemplatePath);
        ...
        else if (relativeWorkspacePath === "README.md" &&
        ...
        await ensureDir(path.dirname(absoluteWorkspacePath));
        await copy(path.join(template.path, relativeTemplatePath), absoluteWorkspacePath);
    }
 ...
 */
async function installSkills(workspace, template, force) {
    ...
    const relevantSkills = SKILL_PACKAGES.filter(({ packageName }) => deps[packageName] !== undefined);
 ...
 */
async function createClaudeMd(workspace, force) {
 ...
 */
async function createDotClaude(workspace) {
 ...
 */
export async function installProjectDependencies({ workspace, template, install, update, formatSuccessMessage = true, }) {
    const pathToWorkspacePackageJson = path.join(workspace, "package.json");
    const workspacePkg = await readJsonFile(pathToWorkspacePackageJson);
    const packageManager = getPackageManager();
    ...
    if (!(await installsPeerDependenciesByDefault(workspace, packageManager))) {
        const templatePeerDependencies = template.packageJson.peerDependencies ?? {};
    ...
    const templateDependencyEntries = Object.entries(templateDependencies).map(([name, version]) => [name, version.replace(/^workspace:/, "")]);
    ...
    if (Object.keys(dependenciesToInstall).length !== 0) {
        ...
        const command = getDevDependenciesInstallationCommand(packageManager, dependenciesToInstall, packageManagerMajorVersion);
        ...
        if (install) {
            ...
            try {
                ...
                ensureError(error);
                throw new HardhatError(HardhatError.ERRORS.CORE.INIT.FAILED_TO_INSTALL_DEPENDENCIES, error);
            }
    ...
    if (dependenciesToUpdate.length !== 0) {
        // Retrieve the package manager specific installation command
        const command = getDevDependenciesInstallationCommand(packageManager, dependenciesToUpdate, packageManagerMajorVersion);
        const commandString = command.join(" ");
...
function showStarOnGitHubMessage() {
    console.log(styleText("cyan", "Give Hardhat a star on GitHub if you're enjoying it! ⭐️✨"));
    console.log();
    console.log(styleText("cyan", "     https://github.com/NomicFoundation/hardhat"));
}
// NOTE: This function is exported for testing purposes only.
export function shouldUpdateDependency(workspaceVersion, templateVersion) {
    ...
    assertHardhatInvariant(templateRange !== null, "All dependencies of the template should have valid versions");
