...
export async function promptForWorkspace() {
    ensureTTY();
    const workspaceResponse = await enquirer.prompt([
        {
            name: "workspace",
            type: "input",
...
export async function promptForMigrateToEsm(absolutePathToPackageJson) {
    ensureTTY();
    const migrateToEsmResponse = await enquirer.prompt([
        {
            name: "migrateToEsm",
            type: "confirm",
...
export async function promptForTemplate(templates) {
    ensureTTY();
    const templateResponse = await enquirer.prompt([
        {
            name: "template",
            type: "select",
...
export async function promptForForce(files) {
    ensureTTY();
    const forceResponse = await enquirer.prompt([
        {
            name: "force",
            type: "confirm",
...
export async function promptForInstall(safelyFormattedCommand) {
    ensureTTY();
    const installResponse = await enquirer.prompt([
        {
            name: "install",
            type: "confirm",
...
export async function promptForUpdate(safelyFormattedCommand) {
    ensureTTY();
    const updateResponse = await enquirer.prompt([
        {
            name: "update",
            type: "confirm",
...
function ensureTTY() {
    if (process.stdout.isTTY !== true) {
        throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.NOT_IN_INTERACTIVE_SHELL);
    }
}
