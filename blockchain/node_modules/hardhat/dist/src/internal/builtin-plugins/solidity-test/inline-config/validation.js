 ...
 */
export function validateInlineOverrides(overrides) {
    const seen = new Set();
    for (const { inputSourceName, contractName, functionName, functionSelector, rawKey, rawValue, key, } of overrides) {
        const functionFqn = getFunctionFqn(inputSourceName, contractName, functionName);
        // Validate key
        if (!Object.hasOwn(KEY_TYPES, key)) {
            throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.INLINE_CONFIG_INVALID_KEY, {
                key: rawKey,
        ...
        if (dotIndex !== -1) {
            ...
            if ((isFuzzTest && keyCategory === "invariant") ||
        ...
        if (seen.has(dedupeKey)) {
            throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.INLINE_CONFIG_DUPLICATE_KEY, { key: rawKey, functionFqn });
        }
        seen.add(dedupeKey);
        // Validate value type
        ...
        if (expectedType === "number") {
            if (!/^(0|[1-9]\d*)$/.test(rawValue) ||
                ...
                throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.INLINE_CONFIG_INVALID_VALUE, {
                    value: rawValue,
                    key: rawKey,
            ...
            assertHardhatInvariant(false, `Unhandled inline config value type for key ${key}`);
        }
    }
}
