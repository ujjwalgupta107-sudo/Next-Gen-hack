...
import { bytesIncludesUtf8String, parseJsonBytes, } from "@nomicfoundation/hardhat-utils/bytes";
 ...
 */
export async function collectEip712CanonicalTypes(buildInfosAndOutputs, inputToUserSource, config) {
    ...
    for (const { buildInfoId, buildInfo, output } of buildInfosAndOutputs) {
        ...
        let parsedOutput;
        try {
            parsedOutput = await parseJsonBytes(output);
        }
        catch (error) {
            ensureError(error);
