...
import { buildInfoContainsInlineConfig, resolveFunctionSelector, buildConfigOverride, getFunctionFqn, } from "./helpers.js";
 ...
 */
export async function getTestFunctionOverrides(testSuiteArtifacts, buildInfosAndOutputs) {
    const allRawOverrides = await collectRawOverrides(testSuiteArtifacts, buildInfosAndOutputs);
    validateInlineOverrides(allRawOverrides.overrides);
    return buildTestFunctionOverrides(allRawOverrides);
}
async function collectRawOverrides(testSuiteArtifacts, buildInfosAndOutputs) {
    const overrides = [];
    const methodIdentifiersByContract = new Map();
    ...
    const artifactIdsByFqn = new Map();
    const buildInfoAndOutputById = new Map(buildInfosAndOutputs.map((bio) => [bio.buildInfoId, bio]));
    for (const edrArtifactWithMetadata of testSuiteArtifacts) {
        const fqn = getFullyQualifiedName(edrArtifactWithMetadata.edrArtifact.id.source, edrArtifactWithMetadata.edrArtifact.id.name);
        ...
        let artifacts = artifactsBySource.get(edrArtifactWithMetadata.edrArtifact.id.source);
        if (artifacts === undefined) {
            artifacts = [];
            artifactsBySource.set(edrArtifactWithMetadata.edrArtifact.id.source, artifacts);
        }
        artifacts.push(edrArtifactWithMetadata);
        artifactIdsByFqn.set(fqn, edrArtifactWithMetadata.edrArtifact.id);
    }
    for (const [buildInfoId, artifactsBySource,] of artifactsGroupedByBuildInfo.entries()) {
        ...
        if (buildInfoAndOutput === undefined) {
            ...
            assertHardhatInvariant(artifacts !== undefined && artifacts.length > 0, "An artifact must be present for the build info");
            const anyArtifact = artifacts[0];
            const fqn = getFullyQualifiedName(anyArtifact.userSourceName, anyArtifact.edrArtifact.id.name);
        ...
        let buildInfoOutput;
        try {
            buildInfoOutput = await parseJsonBytes(buildInfoAndOutput.output);
        }
        catch (error) {
            ensureError(error);
            throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY.BUILD_INFO_OUTPUT_PARSE_ERROR, { buildInfoId }, error);
        }
        for (const [inputSourceName, sourceArtifacts,] of artifactsBySource.entries()) {
            const contractNames = new Set(sourceArtifacts.map((a) => a.edrArtifact.id.name));
            const source = buildInfoOutput.output.sources[inputSourceName];
            const extracted = extractInlineConfigFromAst(source.ast, inputSourceName, contractNames);
            ...
            for (const artifact of sourceArtifacts) {
                ...
                const methodIdentifiers = buildInfoOutput.output.contracts?.[inputSourceName][contractName]?.evm
                    ?.methodIdentifiers;
...
function buildTestFunctionOverrides(collected) {
    const { overrides, artifactIdsByFqn, methodIdentifiersByContract } = collected;
    ...
    for (const override of overrides) {
        const functionFqn = getFunctionFqn(override.inputSourceName, override.contractName, override.functionName);
        const groupKey = override.functionSelector !== undefined
    ...
    for (const [_groupKey, groupOverrides] of overridesByFunction.entries()) {
        ...
        const functionFqn = getFunctionFqn(firstOverride.inputSourceName, firstOverride.contractName, firstOverride.functionName);
        ...
        assertHardhatInvariant(artifactId !== undefined, `Missing artifact id for "${contractFqn}"`);
        ...
        if (firstOverride.functionSelector !== undefined) {
            ...
            assertHardhatInvariant(methodIdentifiers !== undefined, `Missing method identifiers for "${contractFqn}"`);
            selector = resolveFunctionSelector(methodIdentifiers, firstOverride.functionName);
        }
        if (selector === undefined) {
            throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS
                .INLINE_CONFIG_UNRESOLVED_SELECTOR, { functionFqn });
        }
