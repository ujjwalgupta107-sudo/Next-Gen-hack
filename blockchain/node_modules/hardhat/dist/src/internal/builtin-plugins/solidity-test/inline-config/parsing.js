...
import { kebabToCamelCase, snakeToCamelCase, } from "@nomicfoundation/hardhat-utils/string";
import { HARDHAT_CONFIG_PREFIX, FORGE_CONFIG_PREFIX, TOP_LEVEL_KEYS, } from "./constants.js";
import { getFunctionFqn } from "./helpers.js";
/**
 * Extracts raw inline config overrides from a solc AST for a single source
 * file.
 */
export function extractInlineConfigFromAst(ast, inputSourceName, contractNames) {
    ...
    for (const node of nodes) {
        ...
        for (const member of members) {
            ...
            const fnSelector = typeof member.functionSelector === "string"
            ...
            for (const line of docText.split("\n")) {
                const parsed = parseInlineConfigLine(line, inputSourceName, contractName, fnName);
                if (parsed !== undefined) {
 ...
 */
export function parseInlineConfigLine(line, inputSourceName, contractName, functionName) {
    ...
    if (eqIndex === -1) {
        throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.INLINE_CONFIG_INVALID_SYNTAX, {
            line: trimmedLine,
            functionFqn,
        });
    }
