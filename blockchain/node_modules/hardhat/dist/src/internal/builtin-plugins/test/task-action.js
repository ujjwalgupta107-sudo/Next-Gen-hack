...
import { errorResult, isResult, successfulResult, } from "../../../utils/result.js";
...
const runAllTests = async ({ testFiles, chainType, grep, grepExclude, noCompile, ...otherArgs }, hre) => {
    ...
    const subtasksToFiles = testFiles.length !== 0
    ...
    if (passed.length > 0) {
        logSummaryLine("passing", passed, (text) => styleText("green", text));
    }
    if (failed.length > 0) {
        logSummaryLine("failing", failed, (text) => styleText("red", text));
    }
    if (skipped.length > 0) {
        logSummaryLine("skipped", skipped, (text) => styleText("cyan", text));
    }
    if (todo.length > 0) {
        logSummaryLine("todo", todo, (text) => styleText("blue", text));
    }
    ...
    if (hre.globalOptions.gasStats === true ||
        ...
        if (hre.globalOptions.gasStatsJson !== undefined) {
            await gasAnalytics.writeGasStatsJson(hre.globalOptions.gasStatsJson, ...ranSubtaskIds);
        }
    }
    if (hasFailures) {
...
function logSummaryLine(label, items, 
// Allow passing a custom colorize function for testing purposes
color = (text) => styleText("white", text)) {
    let total = 0;
    const str = items
        .map(([name, count]) => {
        total += count;
...
async function registerTestRunnersForFiles(testFiles, hooks) {
    const subtasksToFiles = {};
    const notFound = [];
    for (const file of testFiles) {
        const subtaskName = await hooks.runHandlerChain("test", "registerFileForTestRunner", [file], async (_file) => undefined);
    ...
    if (notFound.length !== 0) {
        throw new HardhatError(HardhatError.ERRORS.CORE.TEST_PLUGIN.CANNOT_DETERMINE_TEST_RUNNER, {
            files: notFound.join(", "),
        });
    }
    return subtasksToFiles;
}
function getTestFilesForSubtask(subtask, testFiles, subtaskToFiles) {
