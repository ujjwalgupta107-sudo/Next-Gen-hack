...
import { bytesToHexString, hexStringToBigInt, hexStringToBytes, } from "@nomicfoundation/hardhat-utils/hex";
import { bytesToBigInt, bytesToNumber, } from "@nomicfoundation/hardhat-utils/number";
import { rpcAddress, rpcAny, rpcData, rpcTransactionRequest, validateParams, } from "@nomicfoundation/hardhat-zod-utils/rpc";
// micro-eth-signer is known to be slow to load, so we lazy load it
let microEthSigner;
let microEthSignerTypedData;
import { getRequestParams } from "../../../json-rpc.js";
import { ChainId } from "../chain-id/chain-id.js";
const EXTRA_ENTROPY = false;
export class LocalAccountsHandler extends ChainId {
    #methods = new Set([
        "eth_accounts",
        "eth_requestAccounts",
        "eth_sign",
    ...
    async #getAddressesAndPrivateKeysMap() {
        if (this.#addresses === undefined ||
            this.#addressToPrivateKey === undefined) {
            const { addresses, addressToPrivateKey } = await this.#initializeAddressesFromPrivateKeys(this.#localAccountsHexPrivateKeys);
    ...
    async #resolveRequest(jsonRpcRequest) {
        ...
        if (jsonRpcRequest.method === "eth_sign") {
            if (params.length > 0) {
                ...
                if (address !== undefined) {
                    if (data === undefined) {
                        throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.ETHSIGN_MISSING_DATA_PARAM);
                    }
                    ...
                    return this.#createJsonRpcResponse(jsonRpcRequest.id, microEthSignerTypedData.personal.sign(data, privateKey, EXTRA_ENTROPY));
                }
            }
        }
        if (jsonRpcRequest.method === "personal_sign") {
            if (params.length > 0) {
                ...
                if (data !== undefined) {
                    if (address === undefined) {
                        throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK
        ...
        if (jsonRpcRequest.method === "eth_signTypedData_v4") {
            ...
            if (privateKey !== null) {
                ...
                return this.#createJsonRpcResponse(jsonRpcRequest.id, microEthSignerTypedData.signTyped(typedMessage, privateKey, EXTRA_ENTROPY));
            }
        }
        return null;
    }
    async #modifyRequest(jsonRpcRequest) {
        const params = getRequestParams(jsonRpcRequest);
        if (jsonRpcRequest.method === "eth_sendTransaction" && params.length > 0) {
            const [txRequest] = validateParams(params, rpcTransactionRequest);
            if (txRequest.gas === undefined) {
                throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.MISSING_TX_PARAM_TO_SIGN_LOCALLY, { param: "gas" });
            }
            if (txRequest.from === undefined) {
                throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.MISSING_TX_PARAM_TO_SIGN_LOCALLY, { param: "from" });
            }
            ...
            if (!hasGasPrice && !hasEip1559Fields) {
                throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.MISSING_FEE_PRICE_FIELDS);
            }
            if (hasGasPrice && hasEip7702Fields) {
                throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.INCOMPATIBLE_EIP7702_FIELDS);
            }
            if (hasGasPrice && hasEip1559Fields) {
                throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.INCOMPATIBLE_FEE_PRICE_FIELDS);
            }
            if (hasEip1559Fields && txRequest.maxFeePerGas === undefined) {
                throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.MISSING_TX_PARAM_TO_SIGN_LOCALLY, { param: "maxFeePerGas" });
            }
            if (hasEip1559Fields && txRequest.maxPriorityFeePerGas === undefined) {
                throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.MISSING_TX_PARAM_TO_SIGN_LOCALLY, { param: "maxPriorityFeePerGas" });
            }
            ...
            const rawTransaction = await this.#getSignedTransaction(txRequest, chainId, privateKey);
            jsonRpcRequest.method = "eth_sendRawTransaction";
            jsonRpcRequest.params = [bytesToHexString(rawTransaction)];
        }
    }
    async #initializeAddressesFromPrivateKeys(localAccountsHexPrivateKeys) {
        if (microEthSigner === undefined) {
            microEthSigner = await import("micro-eth-signer");
        }
        const privateKeys = localAccountsHexPrivateKeys.map((h) => hexStringToBytes(h));
        const addresses = [];
    ...
    async #getPrivateKeyForAddress(address) {
        ...
        if (pk === undefined) {
            throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.NOT_LOCAL_ACCOUNT, {
                account: bytesToHexString(address),
            });
        }
        return pk;
    }
    async #getPrivateKeyForAddressOrNull(address) {
        try {
    ...
    async #getNonce(address) {
        ...
        assertHardhatInvariant(typeof response === "string", "response should be a string");
        return hexStringToBigInt(response);
    }
    async #getSignedTransaction(transactionRequest, chainId, privateKey) {
        if (microEthSigner === undefined) {
            microEthSigner = await import("micro-eth-signer");
        }
        ...
        const checksummedAddress = addr.addChecksum(bytesToHexString(txData.to ?? new Uint8Array()), true);
        ...
        if (authorizationList !== undefined) {
            assertHardhatInvariant(txData.maxFeePerGas !== undefined, "maxFeePerGas should be defined");
