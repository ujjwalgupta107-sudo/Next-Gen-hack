...
import { hardhatMiningIntervalToEdrMiningInterval, hardhatMempoolOrderToEdrMineOrdering, hardhatHardforkToEdrSpecId, hardhatForkingConfigToEdrForkConfig, resolveDefaultTransactionGasLimit, } from "./utils/convert-to-edr.js";
...
export class EdrProvider extends BaseProvider {
        ...
        try {
            ...
            const provider = await context.createProvider(hardhatChainTypeToEdrChainType(networkConfig.chainType), providerConfig, {
                ...
                decodeConsoleLogInputsCallback: (inputs) => {
                    return ConsoleLogger.getDecodedLogs(inputs.map((input) => {
    ...
    async request(requestArguments) {
        ...
        if (this.#jsonRpcRequestWrapper !== undefined) {
            jsonRpcResponse = await this.#jsonRpcRequestWrapper(jsonRpcRequest, this.#handleRequest.bind(this));
        }
        else {
        ...
        if (jsonRpcRequest.method === "web3_clientVersion") {
            assertHardhatInvariant(typeof jsonRpcResponse.result === "string", "Invalid client version response");
    ...
    async #handleEdrResponse(edrResponse, method, params) {
        ...
        if (isFailedJsonRpcResponse(jsonRpcResponse)) {
            ...
            if (stackTrace?.kind === "StackTrace") {
                ...
                assertHardhatInvariant(isEdrProviderErrorData(responseError.data), "Invalid error data");
                error = createSolidityErrorWithStackTrace(responseError.message, stackTrace.entries, responseError.data.data, responseError.data.transactionHash);
            }
            else {
                if (stackTrace !== null) {
                    if (stackTrace.kind === "UnexpectedError") {
                        await sendErrorTelemetry(new EdrProviderStackTraceGenerationError(stackTrace.errorMessage));
                        log(`Failed to get stack trace: ${stackTrace.errorMessage}`);
                    }
                    else {
                        const errHeuristicFailed = "Heuristic failed to generate stack trace";
                        await sendErrorTelemetry(new EdrProviderStackTraceGenerationError(errHeuristicFailed));
    ...
    #emitLegacySubscriptionEvent(subscription, result) {
        this.emit("notification", {
            subscription,
            result,
        });
    }
    #emitEip1193SubscriptionEvent(subscription, result) {
        const message = {
    ...
    async #handleRequest(request) {
        assertHardhatInvariant(this.#provider !== undefined, "The provider is not defined");
        ...
        return await this.#handleEdrResponse(edrResponse, request.method, Array.isArray(request.params) ? request.params : undefined);
    }
}
export async function getProviderConfig(networkConfig, coverageConfig, gasReportConfig, chainDescriptors, includeCallTraces) {
    ...
    const genesisBlockGasLimit = typeof networkConfig.blockGasLimit === "bigint"
    ...
    const miningBlockGasLimit = networkConfig.blockGasLimit === false ? undefined : genesisBlockGasLimit;
    ...
    const forkConfig = await hardhatForkingConfigToEdrForkConfig(networkConfig.forking, chainDescriptors, networkConfig.chainType);
    const network = forkConfig !== undefined
        ? forkConfig
        : {
    ...
    return {
        ...
        mining: {
            ...
            interval: hardhatMiningIntervalToEdrMiningInterval(networkConfig.mining.interval),
            memPool: {
                order: hardhatMempoolOrderToEdrMineOrdering(networkConfig.mining.mempool.order),
            },
        },
        network,
...
//# sourceMappingURL=edr-provider.js.map