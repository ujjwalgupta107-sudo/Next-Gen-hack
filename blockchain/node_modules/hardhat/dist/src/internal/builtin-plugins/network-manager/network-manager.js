...
export class NetworkManagerImplementation {
    ...
    #contractDecoder;
    #getOrCreateMutex = new AsyncMutex();
    #getOrCreateCache = new Map();
    #chainTypesWithSentAnalytics = new Set();
    ...
    async create(networkOrParams) {
        ...
        const networkConnection = await this.#hookManager.runHandlerChain("network", "newConnection", [], async (_context) => await this.#initializeNetworkConnection(networkName, chainType, override));
    ...
    async getOrCreate(networkOrParams) {
        ...
        const { resolvedNetworkName, resolvedChainType } = this.#resolveNetworkAndChainType(network, chainType);
    ...
    async #initializeNetworkConnection(networkName, chainType, networkConfigOverride) {
        const { resolvedNetworkName, resolvedChainType } = this.#resolveNetworkAndChainType(networkName, chainType);
        ...
        const createProvider = async (networkConnection) => {
            const jsonRpcRequestWrapper = (request, defaultBehavior) => hookManager.runHandlerChain("network", "onRequest", [networkConnection, request], async (_context, _connection, req) => await defaultBehavior(req));
            if (resolvedNetworkConfig.type === "edr-simulated") {
                ...
                return await EdrProvider.create({
                    ...
                    networkConfig: {
                        ...applyCoverageNetworkOverrides(resolvedNetworkConfig, shouldEnableCoverage),
                        chainType: resolvedChainType,
                    },
        ...
        return await NetworkConnectionImplementation.create(this.#nextConnectionId++, resolvedNetworkName, resolvedChainType, resolvedNetworkConfig, async (connection) => {
    ...
    #resolveNetworkAndChainType(network, chainType) {
        ...
        const resolvedChainType = chainType ?? existingNetworkConfig.chainType ?? this.#defaultChainType;
        return {
            resolvedNetworkName,
            /* eslint-disable-next-line @typescript-eslint/consistent-type-assertions
            -- The cast is safe because the fallback values are valid chain
            types that match the caller's expected type at runtime. */
            resolvedChainType: resolvedChainType,
        };
    }
