...
import { hasArm64MirrorBuild, hasOfficialArm64Build, missesSomeOfficialNativeBuilds, } from "./build-system/solc-info.js";
import { DEFAULT_OUTPUT_SELECTION, SOLC_DEFAULT_OPTIMIZER_RUNS, } from "./constants.js";
/**
...
const otherSolidityCompilerUserConfigType = z.object(commonSolidityCompilerUserConfigFields);
...
const solcSingleVersionSolidityUserConfigType = solcSolidityCompilerUserConfigType.extend({
    ...commonSolidityUserConfigFields,
    toolVersionsInBuildInfo: z.boolean().optional(),
    ...incompatibleVersionFields,
});
const otherSingleVersionSolidityUserConfigType = otherSolidityCompilerUserConfigType.extend({
    ...commonSolidityUserConfigFields,
    toolVersionsInBuildInfo: z.boolean().optional(),
    ...incompatibleVersionFields,
});
...
export function validateSolidityConfig(resolvedConfig) {
    const errors = [];
    errors.push(...validateRegisteredCompilerTypes(resolvedConfig));
    errors.push(...validatePreferWasmRequiresSolc(resolvedConfig));
    return errors;
}
function validateRegisteredCompilerTypes(resolvedConfig) {
    const errors = [];
    const registered = new Set(resolvedConfig.solidity.registeredCompilerTypes);
    for (const [profileName, profile] of Object.entries(resolvedConfig.solidity.profiles)) {
...
function validatePreferWasmRequiresSolc(resolvedConfig) {
    const errors = [];
    for (const [profileName, profile] of Object.entries(resolvedConfig.solidity.profiles)) {
...
function resolveSolidityConfig(solidityConfig) {
    ...
    if ("version" in solidityConfig || "compilers" in solidityConfig) {
        return {
            profiles: {
                default: resolveBuildProfileConfig(solidityConfig, false, solidityConfig.toolVersionsInBuildInfo),
                production: resolveBuildProfileConfig(copyFromDefault(solidityConfig), true, solidityConfig.toolVersionsInBuildInfo),
            },
    ...
    for (const [profileName, profile] of Object.entries(solidityConfig.profiles)) {
        profiles[profileName] = resolveBuildProfileConfig(profile, profileName === "production", profile.toolVersionsInBuildInfo);
    }
...
function resolveBuildProfileConfig(solidityConfig, production = false, toolVersionsInBuildInfo) {
    ...
    return {
        compilers: solidityConfig.compilers.map((compiler) => resolveSolidityCompilerConfig(compiler, production)),
...
function resolveSolidityCompilerConfig(compilerConfig, production = false) {
    ...
    const resolvedSettings = deepMerge(defaultSettings, compilerConfig.settings ?? {});
    ...
    if (isSolcSolidityCompilerUserConfig(compilerConfig)) {
        ...
        let resolvedPreferWasm = compilerConfig.preferWasm;
...
export function isSolcSolidityCompilerUserConfig(config) {
